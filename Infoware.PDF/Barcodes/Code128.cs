...
public class Code128 : BarCode2
{
    ...
    private readonly byte SwitchToCodC;
    private readonly byte SwitchToCodA;
    private readonly byte SwitchToCodB;
    ...
    private readonly Code_128_Code_Types CurrentCode128Code;
    ...
    private static byte GetCodeCValue(string characters)
    {
        try
        {
            ...
            return result >= 0 && result <= 106 ? result : throw new ArgumentOutOfRangeException(nameof(characters));
        }
        catch (Exception)
        {
    ...
    private byte GetCodeABValue(char character)
    {
        try
        {
            ...
            return result >= 0 && result <= 106 ? result : throw new ArgumentOutOfRangeException(nameof(character));
        }
        catch (Exception)
        {
    ...
    private void RenderBar(BarCodeRenderInfo2 info, double barWidth, XBrush brush)
    {
        ...
        XRect rect = new(info.CurrPos.X, yPos, barWidth, height);
