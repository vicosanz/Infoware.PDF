...
namespace PdfSharpCore.Utils
{
    public class ImageSharpImageSource<TPixel> : ImageSource where TPixel : unmanaged, IPixel<TPixel>
    {
        ...
        private class ImageSharpImageSourceImpl<TPixel2>(string name, Image<TPixel2> image, int quality, bool isTransparent) : IImageSource where TPixel2 : unmanaged, IPixel<TPixel2>
        {
            ...
            {
                BmpEncoder bmp = new() { BitsPerPixel = BmpBitsPerPixel.Pixel32 };
                Image.Save(ms, bmp);
            }
        }
    }
